...
const addToDrinkBasket = (drinkName, drinkPrice) => {
    const drinkItem = {
        name: drinkName,
        price: drinkPrice,
        amount: 1,
    ...
    const existingItem = drinkBasketArray.find(item => item.name === drinkName);

    if (existingItem) {
        existingItem.amount++;
    } else {
        drinkBasketArray.push(drinkItem);
    }
...
document.addEventListener('DOMContentLoaded', function () {
		document.querySelector('.item' + "<%= i+1 %>").addEventListener('click', (event) => {    
			...
			const itemNumber = 1; 
	    const drinkName = event.currentTarget.querySelector('span#drink').innerText; // 혹은 다른 적절한 선택자를 사용하여 메뉴 이름을 가져옵니다.
	    const drinkPrice = event.currentTarget.querySelector('span#don').innerText;;
		
	
		
	    addToBasket(drinkName, drinkPrice);
	    MicroModal.close('modal-1');
