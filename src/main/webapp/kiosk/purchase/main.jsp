...
import="cookie.Cookie_Singleton" %> <%@ page import="cookie.Cookie_Verification_Coupon" %> <%@ page
import="cookie.Cookie_Verification_Smile" %>

<jsp:useBean id="Coupon_Mgr" class="coupon.Coupon_Mgr" />

<% String couponIsCorrect = request.getParameter("couponIsCorrect"); String smileIsCorrect =
request.getParameter("smileIsCorrect"); if (smileIsCorrect == "true" && (smileIsCorrect == null ||
smileIsCorrect.isEmpty())) { Cookie[] cookies = request.getCookies(); Cookie_Singleton cs =
Cookie_Singleton.getInstance(); Cookie_Verification_Smile cvs = new Cookie_Verification_Smile();
cs.setCookieInfo("smile", cookies); cs.deleteCookie(response); cs.verifyCookieByTypeWithNo("value", cvs); if
(cs.isThereCookie()) { cs.addCookie(response); } } if (couponIsCorrect == "true" && (couponIsCorrect == null ||
couponIsCorrect.isEmpty())) { Cookie[] cookies = request.getCookies(); Cookie_Singleton cs =
Cookie_Singleton.getInstance(); Cookie_Verification_Coupon cvc = new Cookie_Verification_Coupon();
cs.setCookieInfo("coupons", cookies); cs.deleteCookie(response); cs.verifyCookie("code", cvc); if (cs.isThereCookie()) {
